...
import com.exa.android.letstalk.data.domain.call.models.*
...
class CallSignalingRepository @Inject constructor(
     ...
     */
    private suspend fun cleanupOldCalls(userId: String, beforeTime: com.google.firebase.Timestamp) {
        try {
     ...
     */
    fun observeIncomingCalls(userId: String): Flow<CallOffer> = callbackFlow {
     ...
     */
    fun observeCallAnswer(callId: String): Flow<CallAnswer?> = callbackFlow {
     ...
     */
    fun observeIceCandidates(callId: String, remotePeerId: String): Flow<IceCandidate> = callbackFlow {
        val listener = callsCollection.document(callId)
            .collection("candidates")
