...
import com.exa.android.letstalk.data.signal_protocol.SignalService
import com.exa.android.letstalk.core.fm.postNotificationToUsers
import com.exa.android.letstalk.core.fm.subscribeForNotifications
...
import com.exa.android.letstalk.core.utils.helperFun.getOtherProfilePic
...
//        return !querySnapshot.isEmpty
    ...
    // Simple AES fallback encryption
    private fun encryptWithAES(plaintext: String): String {
        return try {
            ...
            // Store key and IV with encrypted data (simple approach for fallback)
            val combined = secretKey.encoded + iv + encrypted
            Base64.encodeToString(combined, Base64.NO_WRAP)
        } catch (e: Exception) {
            Log.e(TAG, "AES encryption failed, sending plaintext", e)
            plaintext // Last resort: send plaintext
        }
    }
    
    // Simple AES fallback decryption
    private fun decryptWithAES(ciphertext: String): String {
        return try {
            val combined = Base64.decode(ciphertext, Base64.NO_WRAP)
            
            // Extract key, IV, and encrypted data
            val keyBytes = combined.copyOfRange(0, 32) // 256 bits = 32 bytes
            val iv = combined.copyOfRange(32, 32 + 12) // GCM standard IV size
            val encrypted = combined.copyOfRange(32 + 12, combined.size)
            ...
            ciphertext // Return as-is if decryption fails
        }
    }
    ...
    suspend fun sendMessage(
        ...
        try {
            ...
            val plaintext = message.message
            
            // Get recipients (exclude sender)
            val recipients = message.members.filterNotNull().filter { it != currentUserId && it.isNotEmpty() }
            
            if (recipients.isEmpty()) {
                Log.e(TAG, "❌ No recipients found for message, cannot encrypt")
                throw IllegalStateException("Cannot send message: no recipients")
            }
            
            val recipientUserId = recipients.first()
            
            // Try Signal Protocol encryption first
            var encryptedMessage: Message? = null
            var useSignalProtocol = true
            
            try {
                val senderDeviceId = signalKeyManager.getDeviceId()
                val deviceBundles = firestoreKeyRepo.fetchDeviceKeys(recipientUserId)
                
                if (deviceBundles.isNotEmpty()) {
                    val deviceBundle = deviceBundles.first()
                    val receiverDeviceId = deviceBundle.deviceId
                    
                    // Build session if doesn't exist
                    if (!signalService.hasSession(recipientUserId, receiverDeviceId)) {
                        Log.d(TAG, "Building new session with $recipientUserId:$receiverDeviceId")
                        signalService.buildSessionForRemote(recipientUserId, deviceBundle)
                    }
                    
                    // Encrypt message
                    val ciphertext = signalService.encryptMessage(
                        recipientUserId,
                        receiverDeviceId,
                        plaintext.toByteArray()
                    )
                    
                    encryptedMessage = message.copy(
                        message = Base64.encodeToString(
                    ...
                    Log.d(TAG, "✓ Message encrypted with Signal Protocol")
                } else {
                    throw Exception("No device keys available")
                }
            } catch (e: Exception) {
                Log.w(TAG, "⚠️ Signal Protocol encryption failed: ${e.message}, falling back to AES", e)
                useSignalProtocol = false
                
                // Fallback to simple AES encryption
                val aesEncrypted = encryptWithAES(plaintext)
                encryptedMessage = message.copy(
                    message = aesEncrypted,
                    ciphertextType = -1 // Special flag for AES fallback
                )
            }
            
            // Store encrypted message
            if (encryptedMessage != null) {
                messageRef.document(message.messageId).set(encryptedMessage).await()
                
                if (useSignalProtocol) {
                    Log.d(TAG, "✓ Message sent with Signal E2EE")
                } else {
                    Log.d(TAG, "✓ Message sent with AES fallback encryption")
                }
            } else {
                // Last resort: send plaintext
                Log.w(TAG, "⚠️ All encryption failed, sending plaintext")
            ...
            postNotificationToUsers(
                ...
                messageContent = if (plaintext.isNullOrEmpty()) message.media?.mediaType?.name
                    ?: "Media" else plaintext,
                imageUrl = imageUrl,
                appContext = context
            )

            // Update last message and timestamp in the chat document (use plaintext preview)
            updateLastMessage(message.chatId, plaintext)

            //Update last message cnt for each member
            updateUserLastMessageCnt(message.chatId, message.senderId)

        } catch (e: Exception) {
            Log.e(TAG, "Error in sending message - ${e.message}", e)
            throw e // Re-throw so worker knows it failed
        }
    }
                                ...
                                val decryptedMessages = encryptedMessages.mapNotNull { msg ->
                                    try {
                                        when {
                                            ...
                                            msg.ciphertextType > 0 -> {
                                                try {
                                                    val ciphertextBytes = Base64.decode(
                                                        msg.message,
                                                        Base64.NO_WRAP
                                                    )
                                                    val plaintextBytes = signalService.decryptMessage(
                                                        msg.senderId,
                                                        msg.senderDeviceId,
                                                        ciphertextBytes,
                                                        msg.ciphertextType
                                                    ...
                                                    msg.copy(message = plaintext)
                                                } catch (e: Exception) {
                                                    Log.w(TAG, "Signal decryption failed for ${msg.messageId}, trying AES fallback", e)
                                                    // Try AES fallback
                                                    val decrypted = decryptWithAES(msg.message)
                                                    msg.copy(message = decrypted)
                                                }
                                            }
                                            // AES fallback encryption (ciphertextType = -1)
                                            msg.ciphertextType == -1 -> {
                                                val decrypted = decryptWithAES(msg.message)
                                                msg.copy(message = decrypted)
                                            }
                                            // Plaintext or unknown (ciphertextType = 0)
                                            else -> {
                                                Log.w(TAG, "Message ${msg.messageId} is not encrypted")
                                                msg // Return as-is
                                            }
                                        }
                                    } catch (e: Exception) {
                                        Log.e(
                                            TAG,
                                            "❌ Failed to decrypt message ${msg.messageId}: ${e.message}",
                                            e
                                        )
                                        // Show the message even if decryption fails
                                        msg.copy(message = "[Could not decrypt message]")
                                    }
                                }
